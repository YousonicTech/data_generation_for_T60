...
parser.add_argument('--csv_file', type=str,
                     default="/data1/zdm/T60_500Hz_Data_pt/test/HYBL_total.csv" )
parser.add_argument('--dir_str', type=str,
                    default="/data1/zdm/T60data/VAL_RIR/HYBL_RIR/eight-four/")
parser.add_argument('--save_dir', type=str,
                    default="/data1/zdm/T60_500HZ_Data_wav/val/LiveRecord/eight-four/")
...
for file_name in glob.glob(dir_str + r"/*.wav"):
    ...
    f.close()
    wave_data_mid,sr = librosa.load(file_name,mono=True,sr=16000)
    wave_data_mid = librosa.util.normalize(wave_data_mid,axis=0) *32768
    
    wave_data = wave_data_mid.astype(dtype=np.int16)
    ...
    for audio_samples_np in wave_data:
        ...
        for t in range(int(available_part_num)):
            ...
            #print("data:",data.loc[:,:])
            ################################
            T60_data = data.loc[:, ['T60 AHM:']]
            FB_T60_data = data.loc[:, ['FB T60 AHM:']]
            FB_T60_M_data = data.loc[:, ['FB T60 AHM Mean (Ch):']]
